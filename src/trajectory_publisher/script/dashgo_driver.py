...
# brief document
# author: zph
# mail: 834024599@qq.com
# time: 2019-2-28

# this program is used to control EAI-E1 car under ROS environment.

# to make sure you can run it, pay attention to following things:
# 0. fisrt thing to remember:if you want to stop the E1 car in emergency, press the red button in the back
# 1. connect your computer to E1 car's USB port with USB wire
# 2. make sure you open the power of the E1 car
# 3. make sure you have authority to use the usb port, if not, run this command:"sudo chmod 777 /dev/ttyUSB0"
# 4. attention: ttyUSB0 is an typical value of E1's USB port, your maybe different,if it does, change the value of variable "portname" below
# 5. the program start a node named "/Arduino" under ROS environment
# 6. it subscribes Twist type message named "smoother_cmd_vel", which represent the speed command to E1 car, more specifically, the twist.linear.x represents the linear speed, unit: m/s, and twist.angular.z is angular speed, unit:rad/s
# 7. start another ROS node, publishing Twist type message named "smoother_cmd_vel", then you can control the E1 car
# 8. if you test this program for the first time, it is recommanded to limit both the linear.x and angular.z less than 1, in case the E1 car moves in a rush and leads to injury
# 9 this program is downloaded from: https://github.com/EAIBOT/dashgo_e1
...
class Arduino:
    ...
    def __init__(self, port=portname, baudrate=115200, timeout=0.5):
...
class BaseController:
    def __init__(self, arduino, base_frame):
        ...
        self.rate = float(rospy.get_param("~base_controller_rate", 40))
        ...
        pid_params = dict()
        pid_params['wheel_diameter'] = rospy.get_param("~wheel_diameter", 0.190)
        #orgin:0.162 , author zph change this value by experiment to satisfy the accuracy of odometry
        pid_params['wheel_track'] = rospy.get_param("~wheel_track", 0.326)
        #origin 0.33 , author zph change this value by experiment to satisfy the accuracy of odometry
        pid_params['encoder_resolution'] = rospy.get_param("~encoder_resolution", 730) 
        pid_params['gear_reduction'] = rospy.get_param("~gear_reduction", 0.975)#orign 0.975
        ...
        self.accel_limit = rospy.get_param('~accel_limit', 0.03)
        ...
        self.odomPub = rospy.Publisher('/EAI_e1_car/odom', Odometry, queue_size=5)
	...
	self.voltage_pub = rospy.Publisher('voltage_value', Int16, queue_size=30)
        ...
        # note: ispassed is used to stop car in emergency
    ...
    def poll(self):
		...
		self.emergencybt_pub.publish(-1)
            ...
            odom.pose.pose.position.z = 0 #modified to test
...
class ArduinoROS():
    def __init__(self):
        rospy.init_node('EAI_e1_car', log_level=rospy.DEBUG)
                
        # Cleanup when termniating the node
        rospy.on_shutdown(self.shutdown)
        
        self.port = rospy.get_param("~port", portname)
